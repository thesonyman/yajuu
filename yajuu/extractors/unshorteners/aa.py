...
class AADecoder:
    ...
    def decode(self):
        ...
        # The code has constant start and end parts. We extract the code
        # between these separators.
        ...
        # The chain will be trimmed out as we progress, starting at the end
        while data != '':
            # Trim the begin char
            data = data[len(begin_char):]

            # Find the encoded char
            enc_char = ""
    ...
    def parseJSString(self, s):
        try:
            ...
            # .replace('(','str(')[offset:])
            tmp = (
                s.replace('!+[]', '1').replace('!![]', '1').replace('[]', '0'))
            val = int(eval(tmp))
            return val
        except:
            pass
